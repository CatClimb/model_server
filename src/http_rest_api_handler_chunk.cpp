...
void HttpRestApiHandler::registerAll() {
    ...
    registerHandler(PredictChunk, [this](const HttpRequestComponents& request_components, std::string& response, const std::string& request_body, HttpResponseComponents& response_components) -> Status {
            if (request_components.processing_method == "predictChunk") {
                return processPredictChunkRequest(request_components.model_name, request_components.model_version,
                    request_components.model_version_label, request_body, &response);
            } else {
                SPDLOG_DEBUG("Requested REST resource not found");
                return StatusCode::REST_NOT_FOUND;
...
Status HttpRestApiHandler::processPredictChunkRequest(
    const std::string& modelName,
    const std::optional<int64_t>& modelVersion,
    const std::optional<std::string_view>& modelVersionLabel,
    const std::string& request,
    ...
    OBSERVE_IF_ENABLED(reporterOut->requestTimeRest, requestTime);
    file_name = uuid_to_string(new_uuid);
    std::ofstream content_file(file_name);
    if(content_file.is_open()){
        SPDLOG_DEBUG("info:响应结果写入文件")
        outfile<<*response;
        outfile.close();
    }else{
        SPDLOG_DEBUG("error:文件打开失败")
        return status;
    }
    
    rapidjson::Document data;
    doc.AddMember("code", 200, doc.GetAllocator());
    doc.AddMember("data", file_name, doc.GetAllocator());
    rapidjson::StringBuffer buffer;
    rapidjson::Writer<rapidjson::StringBuffer> writer(buffer);
    doc.Accept(writer);
    *response = new std::string(buffer.GetString())
    return StatusCode::OK;
}
